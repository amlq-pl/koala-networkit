...
PlanarSeparator::PlanarSeparator(const NetworKit::Graph& graph) : graph(graph) {}
...
void PlanarSeparator::run() {
    ...
    if (areConnectedComponentsEligibleForPartition(components)) {
        ...
        auto [lvl, parent] = performBFSAndFindSpanningTree(G);
        ...
        std::partial_sum(verticesAtLevel.begin(), verticesAtLevel.end(), prefixSum.begin());

        // Step 4: Find level l1, and k
        // l1 is the smallest level such that the number of vertices in levels 0..l1 is at least n/2
        NetworKit::node l1 = std::lower_bound(prefixSum.begin(), prefixSum.end(), n / 2) - prefixSum.begin();
        // k is the number of vertices in levels 0..l1
        NetworKit::count k = prefixSum[l1];

        // Step 5: Find levels l0 and l2
        // l0 is the largest level such that the number of vertices in level l0 + 2*(l1-l0) is at most 2*sqrt(k)
        NetworKit::node l0 = 0;
        for (int i = 0; i <= l1; i++) {
        ...
        // l2 is the smallest level such that the number of vertices in level l2 + 2*(l2-l1-1) is at most 2*sqrt(n-k)
        ...
        // Step 6: Remove vertices at level >= l2, contract levels <= l0 into vertex x
        ...
        std::vector<std::vector<NetworKit::node>> childrenH(H.upperNodeIdBound(), std::vector<NetworKit::node>());
        ...
        std::function<NetworKit::node(NetworKit::node)> computeCost = [&](NetworKit::node v) -> NetworKit::node {
            if (childrenH[v].empty()) {
                costsH[v] = 1;
                return 1;
            }
            NetworKit::node cost = 1;
            for (auto c : childrenH[v]) {
                cost += computeCost(c);
            }
            costsH[v] = cost;
            return cost;
        ...
        // Walk v1 to root
        for (auto v = v1; v != NetworKit::none && v != -1; v = parentH[v])
            pathV.push_back(v);

        // Walk w1 to root
        for (auto w = w1; w != NetworKit::none && w != -1; w = parentH[w])
            pathW.push_back(w);
        ...
        for (auto w : pathW) {
            if (ancestorsV.count(w)) { lca = w; break; }
        }
        ...
        int posA = std::find(neighborsOfCycle1.begin(), neighborsOfCycle1.end(), cycle[0]) - neighborsOfCycle1.begin();
        int posC = std::find(neighborsOfCycle1.begin(), neighborsOfCycle1.end(), cycle[2]) - neighborsOfCycle1.begin();
        ...
        for (int i = 1; i < (int)neighborsOfCycle1.size(); i++) {
            ...
            if (pos == posC) { clockwise = true; break; }
        }
        ...
        for (int i = 0; i < cycleSize; i++) {
            ...
            int posU = std::find(neighborsV.begin(), neighborsV.end(), u) - neighborsV.begin();
            int posPrev = std::find(neighborsV.begin(), neighborsV.end(), prev) - neighborsV.begin();
            ...
            for (int j = 0; j < (int)neighborsV.size(); j++) {
                auto w = neighborsV[j];
                if (w == prev || w == u) continue;  // skip cycle neighbors
...
bool PlanarSeparator::areConnectedComponentsEligibleForPartition(NetworKit::ConnectedComponents& components) {
    auto largestComponent = components.extractLargestConnectedComponent(graph, false);
    NetworKit::count n = this->graph.numberOfNodes();

    return largestComponent.numberOfNodes() < 2*n / 3;
}

void PlanarSeparator::findSeparatorFromComponents(NetworKit::ConnectedComponents& components) {
    auto largestComponent = components.extractLargestConnectedComponent(graph, false);
    NetworKit::count n = this->graph.numberOfNodes();

    // in both cases the separator will be empty, so we do not assign it explicitly
    ...
    if (largestComponent.numberOfNodes() < n / 3) {
        ...
        largestComponent.forNodes([&](NetworKit::node v) {
            largestNodes.insert(v);
        });
...
std::pair<std::vector<NetworKit::node>, std::vector<NetworKit::node>> PlanarSeparator::performBFSAndFindSpanningTree(const NetworKit::Graph &G, NetworKit::node startNode = 0) {
    NetworKit::count n = G.numberOfNodes();
    std::vector<bool> vis(n, false);
    std::queue<NetworKit::node> Q;

    std::vector<NetworKit::node> lvl(n, -1);
    std::vector<NetworKit::node> parent(n, -1);
...
std::vector<NetworKit::node> PlanarSeparator::findNumberOfVerticesAtLevel(const std::vector<NetworKit::node>& lvl) {
    NetworKit::count maxLevel = *std::max_element(lvl.begin(), lvl.end());
